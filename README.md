...
The included demo script creates a persistent 100 MiB Alpine Linux root filesystem and boots it directly with Cloud Hypervisor using an Alpine `linux-virt` kernel and a generated initramfs.
...
The demo creates an Alpine build environment from the same minirootfs and installs:
...
It then builds a custom initramfs with:

```text
features="base virtio ext4 kms"
```

This is important because the VM boots from an ext4 root filesystem using virtio storage.
...
The kernel and initramfs are regenerated by the demo.
